...
### InSecTT raw-current validation

After downloading and harmonising `insectt_wsn_power_2023`, validate the raw current scale against Table 1 of the related publication:

```bash
stackwise harmonize insectt_wsn_power_2023 --strict
python scripts/validate_insectt_reference.py
```

The harmonised dataset intentionally reports current and charge, not joules, because the dataset README does not state the PPK II source voltage. The validation script estimates the implied constant source voltage from the publication's independent average-power table without modifying the raw-derived observations.

...
## v0.1.4 InSecTT adapter validation

The InSecTT adapter is covered by synthetic nested-ZIP regression tests for technology mapping, 10 us timestamp scaling, period-to-payload mapping, current/charge aggregation, and incomplete-trace rejection. Full raw InSecTT data are intentionally not included in the repository; the user-side `--strict` harmonisation and Table 1 scale-validation script provide the full-data integration check.

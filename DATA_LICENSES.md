...
## LoRaWAN LR-FHSS End-Device Current Consumption Measurements

- Registry ID: `lorawan_lrfhss_energy_2024`
- Zenodo record: `13838241`
- Dataset DOI: `10.5281/zenodo.13838241`
- Related publication DOI: `10.3390/s24175770`
- Licence: Creative Commons Attribution 4.0 International (CC BY 4.0), explicitly reported by the live Zenodo metadata
- Redistribution: permitted with attribution; STACKWISE still keeps external raw files out of Git
- Raw measurements: eight radio-interface current traces, ACK/noACK x DR8--DR11, sampled every 20.48 us
- Measurement boundary: LR1121 radio interface only, not the complete end device
- Source conditions: 4-byte FRM payload and +14 dBm transmit power
- Voltage provenance: the associated publication documents a dedicated 3.3 V supply for the radio interface; this value is used for derived full-trace joules and is explicitly provenance-tagged
- Caution: full-trace energy is not automatically treated as per-message energy. Transaction-level use requires validation of the number and boundaries of transmissions in each capture.


## STACKWISE Empirical Evidence Benchmark v1.0.0

- STACKWISE-authored benchmark tables, metadata, schemas, benchmark definitions and synthetic/model-derived sensitivity artifacts are licensed under **CC BY 4.0**.
- The repository software remains separately licensed under Apache-2.0.
- Upstream datasets remain independent works and must be cited using `SOURCE_ATTRIBUTION.csv`; the final benchmark package does not mirror their raw archives.
- The CC BY 4.0 benchmark licence does not erase or replace upstream attribution obligations.

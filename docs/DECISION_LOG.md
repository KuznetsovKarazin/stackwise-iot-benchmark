...
## ADR — Stage 6C treats PDP-session lifetime as an unweighted deployment uncertainty, not a probability

**Decision.** Use current 1NCE Platform-2.0 documentation to model kB billing rounding at PDP-session closure. Retain two explicit anchors: a long-running PDP context and one PDP context per report. Do not infer occurrence probabilities and do not equate application-security session lifetime with PDP-session lifetime.

**Rationale.** The operator now documents the billing aggregation boundary, resolving the Stage-5I ambiguity, but the benchmark does not identify the deployment's PDP-context lifecycle. A finite unweighted family is therefore admissible whereas a single guessed billing profile or invented probability distribution is not.

**Consequence.** `lifecycle_cost_eur` is decision-usable as `READY_ROBUSTNESS_FAMILY` for the preferred four-candidate subset. Energy remains the only mandatory first-slice blocker.

## 2026-08-12 — Stage 6D decision-engine semantics

Decision: validate the first decision engine before real energy acquisition, but quarantine all synthetic energy/rank outputs from scientific results. Stage-6C cost states and stakeholder-weight anchors are enumerated without probabilities and are summarised only by envelopes. Rank acceptability may have a probabilistic interpretation only within a future justified empirical/bootstrap energy layer conditional on a declared cost state and preference anchor. Alternative-set min/max normalisation is prohibited; fixed value-function anchors are required. Exact ties receive fractional rank mass rather than random tie-breaking.


## DR — Benchmark dataset release before additional laboratory work

Decision: release a derived **STACKWISE Empirical Evidence Benchmark** from the validated public measurements before considering optional new physical measurements. The benchmark must not be described as a newly collected experimental dataset. Raw upstream archives are excluded; derived evidence is redistributed only when the upstream licence is verified. Empirical-derived and synthetic/model-derived layers remain physically and semantically separate.


## D-EXP1-01 — Test ordering with a structural preference envelope, not imputed performance
**Date:** 2026-08-13

**Decision.** Experiment 1 compares score-first and feasibility-first using only four completely observed structural preference features from frozen Benchmark v1.0.0 and 35 deterministic simplex anchors. Missing energy, lifecycle cost, reliability, latency and coverage values are not imputed.

**Reason.** The scientific question is whether hard constraints may be compensated or bypassed by soft scoring. Using missing empirical metrics would confound this ordering question with evidence completion assumptions.

**Consequence.** Anchor counts measure declared stress-test coverage, not probabilities over stakeholders. The experiment supports the feasibility-before-preference claim but does not constitute a real performance MCDA.

## D-EXP1-02 — Never force a decision when the feasible set is empty
**Date:** 2026-08-13

**Decision.** `unresolved` remains distinct from `infeasible`, but neither is admitted to the feasibility-first soft-ranking set. If no candidate has frozen status `feasible`, the output is `NO_FEASIBLE_DECISION`.

**Reason.** A ranking method that always emits a winner creates false decisiveness when hard evidence has not established any admissible alternative.


## D-EXP2-01 — Source provenance grade must not be used as decision admissibility
**Date:** 2026-08-13

**Decision.** Experiment 2 treats A/B/C/D `source_grade` only as provenance/accessibility quality. Decision admissibility is evaluated separately through target relation class and downstream canonical readiness.

**Reason.** All four core datasets are Grade A, yet none directly identifies any of the five canonical decision targets at the frozen target boundary. A source-grade-only rule would therefore massively overstate usable decision evidence.

**Consequence.** Grade thresholds may be reported as a negative ablation, but may not authorise scoring or pooling.

## D-EXP2-02 — Relaxed admissibility regimes are counterfactual diagnostics, not MCDA inputs
**Date:** 2026-08-13

**Decision.** `CONTEXT_ONLY`, structural-transfer, profile-unresolved, incompatible or missing evidence may be progressively counted in diagnostic ablation regimes solely to measure apparent decision-space inflation. They do not become canonical criterion values.

**Reason.** The experiment is intended to quantify the methodological cost of treating evidence availability as evidence admissibility, not to fill gaps by assumption.

**Consequence.** D2/D3 experiment regimes are explicitly marked counterfactual, have no probability interpretation and cannot produce publication candidate rankings.

## 2026-08-13 — Preserve native uncertainty semantics in publication Experiment 3

Decision: do not force Vomhoff bootstrap, LoED campaign/block robustness and lifecycle-cost deployment states into one probability distribution. Use point-vs-native-uncertainty case studies and preserve paired state alignment. Rationale: the three evidence families support different inferential claims, and collapsing them would create unsupported probability statements or false reversal ambiguity.


## D-EXP4-01 — Measure simplification on one aligned state space
**Date:** 2026-08-13

**Decision.** Compare application-only, transport-aware, serialization-aware, session/control-aware and billing-aware accounting on the same 288 Stage-6C traffic/billing states. L0–L3 are deterministic simplifications; L4 is the final billing-aware reference.

**Reason.** Comparing different state sets would confound accounting detail with scenario composition. Aligned states isolate the methodological cost of each simplification layer.

**Consequence.** State-space fractions are deterministic sensitivity coverage, not probabilities. No transport detail is reopened.

## D-EXP4-02 — Separate raw-volume simplification from billing-aware lifecycle cost
**Date:** 2026-08-13

**Decision.** L0–L3 estimate TopUps from aggregate raw volume only; L4 uses the Stage-6C PDP-session billing model. Cost underestimation is reported relative to L4 while holding procurement and tariff prices fixed.

**Reason.** This directly measures the economic bias introduced by omitting protocol/session/billing structure without pretending that simplified raw volume is the operator's actual billed volume.

## Decision — freeze Experiments 1–4 and separate broad vs narrow article scope (v0.1.57)

Do not force a global stochastic MCDA ranking by imputing blocked energy. Treat the matched cellular whole-device energy experiment as optional future validation. The current evidence supports a strong narrow article. If the heterogeneous-fleet claim is retained, add one final fleet portfolio feasibility/simplification experiment using only frozen hard-feasibility evidence and no synthetic performance/cost inputs.

## 2026-08-13 — Fleet portfolio experiment uses set cover, not smoke cost optimisation

Decision: close the heterogeneous-fleet contribution with a minimum-cardinality serviceability portfolio over the frozen hard-feasibility matrix. Do not use the legacy configured fleet prices or synthetic soft scores. Primary coverage treats only `feasible` cells as serviceable; `unresolved` is retained solely as an optimistic closure sensitivity. This preserves the evidence-first contract while quantifying the structural penalty of single-technology simplification.

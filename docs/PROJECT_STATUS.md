...
## Current publication checkpoint — v0.1.55

- `STACKWISE Empirical Evidence Benchmark v1.0.0` is frozen and final-release QA passed 25/25.
- Experiment 1 (feasibility-first vs score-first) is closed: 193/245 score-first scenario-anchor top sets contain at least one hard-infeasible candidate; 142/175 when restricted to scenarios with at least one feasible candidate.
- Experiment 2 (evidence-grade/admissibility ablation) is closed: all 398 records are retained under Grade-A-only provenance filtering, yet the 20 source×canonical-target relations are 0 direct / 5 bridgeable / 1 conditional / 14 missing.
- For the 42 mandatory energy+cost candidate×criterion cells, only 4 are canonical-ready robustness-family cost cells; no candidate is complete under authorised evidence. Counterfactually treating context+structural-transfer support as score-ready inflates the apparent complete set to 10 candidates across 3 scenarios; assumption priors inflate it to all 21 feasible candidates.
- No real publication ranking is authorised by Experiment 2.
- Experiment 3 (deterministic vs uncertainty-/robustness-aware treatment) is closed: all 3 source-aligned Vomhoff point-order pairs are separated by marginal bootstrap intervals; LoED median SD max/min ratios across 3/7/14-day block models range from 1.39 to 1.70 while the point estimate is unchanged; Stage-6C paired cost states show CoAP cheaper in 172/288 RAT-state comparisons, tied in 116/288, and MQTT cheaper in 0/288 despite overlapping marginal cost ranges.
- No cross-block or epistemic-state pooled probability is reported and no global real-candidate ranking is authorised.
- Experiment 4 (accounting/cost simplification ablation) is closed on one aligned 288-state traffic/billing space: nominal-allowance exceedance progresses 0 → 144 → 152 → 216 → 252 states from payload-only to billing-aware accounting. Payload-only produces 252 false-within classifications versus the final reference; transport-aware leaves 108, serialization-aware 100, and session/control-aware 36. Across 576 procurement-expanded rows, payload-only understates five-year connectivity cost by median/max 50/100 EUR.
- Next publication step: consolidate Experiments 1–4 into the main result tables/figures and design the fleet-level simplification experiment without reopening frozen transport accounting.
...
## Stage 6C — lifecycle-cost robustness family — v0.1.48

Stage 6C closes the cost-side blocker for the four-candidate periodic-tracking IP-cellular development subset. Current 1NCE Platform-2.0 documentation resolves the former billing-aggregation ambiguity by specifying kB rounding per PDP session at session end. STACKWISE therefore crosses the frozen Stage-5N protocol/session rows with two unweighted PDP-session anchors (persistent PDP vs one PDP session per report) and two dated BG95 procurement observations (quantity 250 vs quantity 1).

The resulting family contains 576 EUR lifecycle-cost rows. All four preferred candidates are now `READY_ROBUSTNESS_FAMILY` for `lifecycle_cost_eur`. CoAP/DTLS candidates span EUR 39.06644–96.41/device over five years; MQTT/TLS candidates span EUR 49.06644–146.41. Under the shared dual-mode BG95 reference and the same 1NCE tariff, NB-IoT and LTE-M have identical cost families within a binding. No probabilities are assigned.

Energy/report remains blocked for all four candidates, so the first decision slice is still not scoreable and publication MCDA remains prohibited. See `docs/LIFECYCLE_COST_ROBUSTNESS_FAMILY.md`.

## Stage 6D — synthetic nested decision-engine dry run — v0.1.49

**Status: closed for software validation.** The preferred four-candidate periodic-tracking subset is aligned across 144 Stage-6C lifecycle-cost states. Three paired synthetic energy fixtures and 21 deterministic preference anchors validate the nested decision engine without creating real scientific scores. Cost states and weights remain unweighted; rank acceptability is never pooled across those epistemic/sensitivity dimensions. Fixed value functions, fractional tie handling and alternative-order invariance are regression-tested. All 13 engine invariants pass.

Real candidate ranking remains prohibited because matched whole-device `energy/report` is still absent. The next publication-critical action is the Stage-6B five-block energy pilot; Stage-6D code should now remain frozen unless pilot integration reveals a material interface defect.


## Benchmark Dataset Release Candidate — v0.1.50

### v0.1.50.post1 release-builder correction

The benchmark RC content is unchanged. A release-builder bug was fixed: the L3 scenario table is now materialised from the canonical Stage-4E refined scenario definition (7 scenarios) instead of copying the stale Stage-4D six-row CSV. The frozen 7×9 = 63 feasibility matrix and 21/39/3 outcome are unchanged.

**Status: implemented; local production build required.** STACKWISE now treats the harmonised benchmark itself as a scientific deliverable rather than only an intermediate input to MCDA. The release builder packages the validated core-four evidence layer, compact source-specific analysis-ready derivatives, uncertainty/dependence contracts, benchmark scenarios/stacks, the frozen 21/39/3 feasibility matrix, support/readiness tables and clearly segregated synthetic sensitivity artifacts.

The release contains no upstream raw archives and fails closed unless all four core-source licences are verified as redistributable. LoED licence metadata is updated to CC-BY-4.0 from the current Zenodo record. The expected production release is 398 Grade-A evidence records / 4 empirical sources / 14 metrics / 7 scenarios / 9 stacks / 63 hard-feasibility rows.

The benchmark release is `v1.0.0-rc1`, not yet final. Zenodo upload remains blocked until the user runs the builder against the full local data tree and manually reviews the generated release manifest, source attribution, file sizes and checksums.

### v0.1.50.post2 release QA and self-description

The first full local RC build succeeded at 25 table/data artifacts and approximately 2.95 MB, confirming that the benchmark is compact enough to remain a single release package. Manual inspection exposed one packaging defect: Parquet row counts were shown as zero in `RELEASE_TABLE_MANIFEST.csv` because the row counter used an empty-column Pandas read. This is corrected using Parquet metadata.

The RC now packages a benchmark dataset card, eight canonical JSON schemas and all four upstream dataset cards. A dedicated QA audit checks semantic equivalence of the 398-record CSV/JSONL/Parquet evidence representations, complete scenario×stack coverage of the 7×9 feasibility matrix, manifest row counts, checksum coverage, source-licence gates and absence of raw external archives.

Scientific content remains `v1.0.0-rc1`: 398 evidence records / 4 real empirical sources / 14 metrics / 7 scenarios / 9 candidate stacks / 21 feasible / 39 infeasible / 3 unresolved. Final Zenodo publication is still blocked only by explicit owner decisions/manual sign-off, including declaration of the licence for STACKWISE-authored benchmark material; this licence is not inferred automatically from upstream CC-BY metadata.

### v0.1.51 final benchmark freeze

`STACKWISE Empirical Evidence Benchmark v1.0.0` is the frozen benchmark dataset release. Scientific table content is unchanged from the validated RC: 398 canonical evidence records / 4 real empirical sources / 14 metrics / 7 scenarios / 9 candidate stacks / 21 feasible / 39 infeasible / 3 unresolved. STACKWISE-authored benchmark material is CC BY 4.0; software remains Apache-2.0. Four-source attribution is explicitly materialised and reviewed. The benchmark package is ready for final archival QA/deposit, while real publication MCDA remains a separate downstream result.


### v0.1.52 Experiment 1 — feasibility-first vs score-first

**Status: closed.** The frozen Benchmark v1.0.0 is now used for the first publication-oriented result. A 35-anchor, four-feature deterministic structural-preference envelope produces 245 scenario-anchor evaluations. Score-first top sets contain a hard-infeasible candidate in 193/245 cases and are entirely infeasible in 159/245. In the five scenarios with feasible alternatives, the counts remain 142/175 and 115/175. In the two scenarios with no feasible candidate, score-first nevertheless returns a top set in 70/70 anchors; feasibility-first returns no decision. No missing empirical performance metric is filled, and the experiment is not a real candidate MCDA.

## v0.1.57 — publication result consolidation

Experiments 1–4 are frozen as the primary current publication-results block. Five claims are publication-authorised; global stochastic ranking, matched cellular report-energy ranking and fleet-level simplification remain unauthorised. A narrow article can proceed without those claims. One final fleet portfolio feasibility experiment is recommended if the original fleet contribution is retained.

## v0.1.57 — Experiment 5 fleet portfolio feasibility

Experiment 5 closes the original fleet-level structural contribution without introducing new soft evidence. The primary strict universe contains five scenario classes with at least one feasible candidate; two additional benchmark scenarios remain unresolved-only. A single stack, access technology or access family covers at most 4/5 strict-serviceable classes, a 20% serviceability loss. Complete strict coverage requires two stacks / two technologies / two families. Six minimum stack portfolios exist; the technology-level minimum is LTE-M plus either LoRaWAN-LoRa or LoRaWAN-LR-FHSS, and the family-level minimum is uniquely cellular + LoRaWAN. An optimistic feasible+unresolved sensitivity requires three portfolio elements to cover all seven classes and adds Thread. No lifecycle-cost, device-count, soft-score or probability optimisation is performed.

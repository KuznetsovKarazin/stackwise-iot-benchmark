...
1. Run the full local test suite: `pytest -q`.
...
9. Create the immutable tag `v0.1.60` and a GitHub Release from that tag.

The public repository intentionally excludes local `data/`, `results/`, `release/`, `dist/`, `backups/`, `payload/`, root-level ZIP diagnostics and the manuscript workspace. The benchmark itself is already archived at https://doi.org/10.5281/zenodo.21937093.

# STACKWISE research plan — active roadmap after v0.1.53

## Current publication experiment sequence

- **Benchmark v1.0.0:** frozen and release-QA complete.
- **Experiment 1:** feasibility-first vs score-first — closed in v0.1.52.
- **Experiment 2:** source-grade/admissibility ablation — closed in v0.1.53.
- **Next:** Experiment 3, deterministic point treatment versus uncertainty-/robustness-aware treatment, restricted to evidence subsets where uncertainty semantics are scientifically identified. No missing cellular energy values may be imputed.
- **Then:** Experiment 4, cost/accounting simplification ablation using application-payload-only, transport-aware, serialization-aware and session-aware accounting.
- **Then:** fleet-level optimisation/cost-of-simplification using only scenario/criterion combinations with an explicit evidence regime.

The optional matched cellular physical experiment remains future validation, not a blocker for the benchmark/framework paper.
...
**Status: closed.** The preferred periodic-tracking IP-cellular 2×2 subset now has a finite, unweighted EUR lifecycle-cost family. Current 1NCE Platform-2.0 documentation identifies PDP-session-level kB rounding, so the former billing-aggregation ambiguity is replaced by two deployment anchors rather than an unconstrained unknown. Dated single-unit and quantity-250 BG95 prices provide the procurement dimension.

Cost is `READY_ROBUSTNESS_FAMILY` for 4/4 preferred candidates; energy remains blocked for 4/4. Do not extend transport accounting further. The next publication-critical action is the Stage-6B matched whole-device energy pilot. Software development may proceed with a score-free robustness/stochastic-engine dry run using synthetic energy fixtures only in tests; such fixtures must never enter publication results.

## Stage 6D — decision/robustness engine dry run — v0.1.49

**Status: closed.** The decision engine is now validated with synthetic paired energy fixtures only. Keep the Stage-6C 144-state cost envelope and stakeholder-weight grid unweighted; do not average either into a global probability. Do not publish synthetic rank outputs.

**Next:** execute the Stage-6B five-block matched whole-device energy pilot. After pilot quality/variance review, freeze the main replication count and integrate paired empirical/bootstrap energy draws into the Stage-6D engine. Only that integration may open the first real decision experiment.


## Benchmark release track — v0.1.50

Before further real decision experiments, freeze and use **STACKWISE Empirical Evidence Benchmark v1.0.0** as the canonical derived research dataset built from the four validated real measurement sources. The final benchmark preserves source boundaries and uncertainty semantics, keeps synthetic/model-derived sensitivity outputs in a separate layer, and carries explicit source attribution and CC BY 4.0 licensing for STACKWISE-authored benchmark material.

After a clean production build and manual review, freeze `v1.0.0` of the benchmark and use that immutable benchmark version as the input to final evidence-availability, feasibility-first, uncertainty-aware and cost-of-simplification experiments. The optional matched cellular energy experiment remains future validation, not a prerequisite for releasing the benchmark itself.


## Experiment 1 — feasibility-first vs score-first — v0.1.52

**Status: closed.** Use frozen Benchmark v1.0.0 only. The result quantitatively demonstrates the ordering failure of score-first evaluation under a deterministic structural-preference envelope without imputing missing performance evidence.

**Next: Experiment 2 — evidence-grade ablation.** Compare decision-space coverage and conclusion stability under progressively relaxed evidence admission regimes (strict empirical/direct evidence first, then explicitly broader admissibility classes). The primary outputs should be evaluable-candidate coverage, evidence-dependence of conclusions and rank/selection reversals where a valid comparison exists. Do not fabricate values merely to force complete rankings.


## Experiment 2 — evidence-grade and decision-admissibility ablation — v0.1.53

**Status: closed.** Provenance grade and target-level admissibility are now explicitly separated. All 398 core records remain under Grade-A-only filtering, but source×target support is 0 direct / 5 bridgeable / 1 conditional / 14 missing. Canonical energy+cost completeness remains 0 candidates; counterfactual structural-transfer treatment inflates this to 10, and assumption-prior completion to 21. None of the relaxed regimes is publication-score authorised.

## Experiment 3 — deterministic vs uncertainty-/robustness-aware treatment — v0.1.54

**Status: closed.** The experiment retains the native uncertainty semantics instead of coercing all evidence into one stochastic model. Vomhoff uses nonparametric epistemic intervals for conditional source means; LoED retains fixed campaign × temporal block-model robustness; lifecycle cost retains aligned finite deployment/procurement/billing states. A single point can be confirmed (Vomhoff), hide model-form uncertainty (LoED), or overstate a fixed decision margin (cost). Paired state alignment is mandatory; marginal-range overlap must not be interpreted as a reversal probability.

**Status: closed in v0.1.55.** Experiment 4 aligns the frozen Stage-5L–6C artifacts onto one 288-state periodic-tracking traffic/billing space. The accounting ladder exposes 0 → 144 → 152 → 216 → 252 nominal-allowance exceedances and reduces false-within classifications versus billing-aware accounting from 252 to 108 to 100 to 36 before final billing.

**Next: publication-result consolidation and fleet-level cost-of-simplification.** Freeze Experiments 1–4 result tables/figures, then build a fleet experiment using only authorised feasibility/cost evidence. Do not reopen transport accounting or impute blocked energy solely to force a global ranking.

## Publication-result consolidation — v0.1.57

**Status: closed.** Experiments 1–4 are frozen. Five core publication claims are authorised; global candidate ranking and matched cellular report-energy ranking are not.

**Recommended next:** Experiment 5 — fleet portfolio feasibility and simplification penalty. Compute minimum stack/technology portfolio cardinality and serviceability loss under single-stack/single-technology and at-most-two restrictions using only the frozen 7×9 hard-feasibility matrix. Do not add energy/cost assumptions. If this experiment is not performed, remove the heterogeneous-fleet optimisation claim from the paper.

## Stage 7 / Experiment 5 — fleet portfolio feasibility

**Status: implemented in v0.1.57.** Use the frozen Stage-4E 7×9 feasibility matrix as a deterministic set-cover problem. Primary results are restricted to scenarios with at least one strict `feasible` candidate. Report the 20% serviceability loss of the best single-option portfolio, the two-element minimum complete portfolio, and the exact minimum portfolio families. Keep `unresolved` as a separate optimistic evidence-closure sensitivity. Do not introduce synthetic energy, lifecycle prices, device-count weights or stakeholder probabilities. After local confirmation, rerun publication consolidation and decide final article split/submission strategy.
